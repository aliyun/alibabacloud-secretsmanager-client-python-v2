...
def get_version():
    """读取版本号"""
    try:
        init = read("src", "alibaba_cloud_secretsmanager_client_v2", "__init__.py")
...
def get_ca_files():
    """获取CA目录下的所有PEM文件"""
    ca_files = []
    ca_dir = os.path.join(HERE, "src", "alibaba_cloud_secretsmanager_client_v2", "ca")
    try:
...
setup(
    name="aliyun-secret-manager-client-v2",
    packages=find_packages("src", include=["alibaba_cloud_secretsmanager_client_v2",
                                           "alibaba_cloud_secretsmanager_client_v2.*"]),
    package_dir={"": "src"},
    version=get_version(),
    ...
    keywords=["aliyun", "kms", "secrets-manager", "secrets", "v2"],
    ...
    package_data={
        "alibaba_cloud_secretsmanager_client_v2": get_ca_files()
    },
    classifiers=[
